...
import torch
import yaml
from Second_sensor_structure import PhotonicSensor
    ...
    with h5.File(save_path, "w") as f:
        ...
        for stage_idx, stage in enumerate(stages):
            for i in range(sample_per_stage):
                sensor = PhotonicSensor(N = config.structure["N"], M = config.structure["M"], angle=config.structure["angle"],mode=config.structure["mode"], device=device, materials=config.materials, thicknesses=config.thicknesses, wavelength_nm=wavelength, stage = stage)
