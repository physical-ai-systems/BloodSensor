...
class CoupledResonatorMalariaSensor:
    def __init__(self, wavelengths_nm, stage, angle_deg, mode, N, M, si_thickness_nm, sio2_thickness_nm, defect_thickness_nm, device, reference_stage):
        self.device = device
        self.reference_stage = reference_stage.lower()
        self.wavelengths_nm = wavelengths_nm.to(self.device)
        self.stage = stage.lower()
        self.angle_deg = angle_deg
        self.angle_rad = torch.tensor(angle_deg, dtype=torch.float64, device=self.device) * torch.pi / 180.0
        ...
        self.N = N
        self.M = M
        self.si_thickness_nm = si_thickness_nm
        self.sio2_thickness_nm = sio2_thickness_nm
        self.defect_thickness_nm = defect_thickness_nm
        self.si_model = SiDispersion()
        self.sio2_model = SiO2Dispersion()
        self.n_si = self.si_model.calculate_ri(self.wavelengths_nm).to(self.device)
        self.n_sio2 = self.sio2_model.calculate_ri(self.wavelengths_nm).to(self.device)
        self.n_air = torch.ones_like(self.wavelengths_nm)
        self.n_defect_1 = MalariaBloodRefractiveIndex(self.wavelengths_nm, self.stage).get_effective_ri().to(self.device)
        self.n_defect_2 = MalariaBloodRefractiveIndex(self.wavelengths_nm, self.reference_stage).get_effective_ri().to(self.device)
    ...
    def add_layer(self, M_total, n_previous, theta_previous, n_layer, thickness_nm):
        theta_layer = self.snells_law(n_previous.to(torch.complex128), n_layer.to(torch.complex128), theta_previous)
        M_layer = self.layer_matrix(n_layer.to(torch.complex128), thickness_nm, theta_layer)
        M_total = self.multiply(M_total, M_layer)
        return (M_total, n_layer, theta_layer)
    ...
    def build_transfer_matrix(self):
        ...
        theta_previous = torch.full_like(wavelength, self.angle_rad, dtype=torch.complex128)
        M_total = torch.eye(2, dtype=torch.complex128, device=self.device).unsqueeze(0).repeat(wavelength.shape[0], 1, 1)
        (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_sio2, self.sio2_thickness_nm)
        for _ in range(self.N):
            (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_si, self.si_thickness_nm)
            (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_sio2, self.sio2_thickness_nm)
        (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_defect_1, self.defect_thickness_nm)
        for _ in range(self.M):
            (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_sio2, self.sio2_thickness_nm)
            (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_si, self.si_thickness_nm)
        (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_sio2, self.sio2_thickness_nm)
        (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_defect_2, self.defect_thickness_nm)
        (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_sio2, self.sio2_thickness_nm)
        for _ in range(self.N):
            (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_si, self.si_thickness_nm)
            (M_total, n_previous, theta_previous) = self.add_layer(M_total, n_previous, theta_previous, self.n_sio2, self.sio2_thickness_nm)
