import torch
import copy
from PhotonicDataSetGeneration.MaterialsRI import SiO2Dispersion, SiDispersion
from PhotonicDataSetGeneration.WholeBloodRI import MalariaBloodRefractiveIndex

class PhotonicSensor:
    def __init__(self, N, M, angle, mode,device, materials, thicknesses, wavelength_nm,stage):
        self.N = N
        self.M = M
        self.angle = angle
    ...
    def TransferMatrixMethod(self, stage):
        ...
        material_to_ri = {
            self.materials["layer_a"]["name"]: self.ri_layer_a,
            self.materials["layer_b"]["name"]: self.ri_layer_b,
            self.stage: self.ri_malaria
        }
        ...
        inc_material = type('Material', (object,), {
            'refractive_index': torch.tensor(1.0, device=device)
        })()
        full_stack.append(type('Layer', (object,), {'material': inc_material, 'thickness': None})())
        
        for layer in self.layers:
            material_name = layer["material"]
            if material_name in material_to_ri:
                material = type('Material', (object,), {'refractive_index': material_to_ri[material_name]})()
            else:
